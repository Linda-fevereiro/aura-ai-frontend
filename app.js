...
const API_BASE_URL = 'http://localhost:3000/api'; // Mantenha esta URL se estiver rodando o backend localmente na porta 3000
...
function showMessageModal(title, message, type = 'info') {
    ...
    const modalOverlay = document.getElementById('messageModalOverlay'); // Adicionado

    modalTitle.textContent = title;
    modalBody.textContent = message;

    modal.classList.remove('info', 'error', 'success'); // Remove classes de tipo anteriores
    modalOverlay.classList.add('active'); // Ativa o overlay
    modal.classList.add(type); // Adiciona a classe de tipo para estilização
...
class AuraMemory {
    constructor() {
        ...
        // As bases de dados reais (users, chats, knowledgeBase) agora estão no backend/Firestore
        // Mantemos algumas propriedades para estado temporário no frontend
        this.learningBuffer = [];
        this.contextMemory = {};
    ...
    async registerUser(name, email, password) {
        try {
            ...
            return data; // Retorna { user: { uid, email, displayName }, token }
        } catch (error) {
    ...
    async createChat(userId, title) {
        try {
            const response = await fetch(`${API_BASE_URL}/chats`, {
                ...
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${localStorage.getItem('userToken')}` // Envia o token de autenticação
                },
    ...
    async getUserChats(userId) {
        try {
            ...
            return data; // Retorna um array de chats
        } catch (error) {
    ...
    async getChatMessages(chatId) {
        try {
            ...
            return data; // Retorna um array de mensagens
        } catch (error) {
    ...
    processLearningBuffer() {
        // Em um app real, você enviaria esses dados para um endpoint de aprendizado no backend
        // Ex: fetch(`${API_BASE_URL}/learn`, { method: 'POST', body: JSON.stringify(this.learningBuffer) });
    ...
    generalizeKnowledge(sourceDomain, targetDomain) {
        // Lógica de generalização mais complexa exigiria um modelo de IA no backend
    ...
    async queryKnowledge(message) {
        try {
            ...
            const data = await response.json();
            if (!response.ok) {
                throw new Error(data.message || 'Erro ao consultar conhecimento.');
            }
...
class AuraAIController {
    ...
    async loadChat(chatId) {
        ...
        if (messages) {
            ...
            // Não é necessário chamar loadUserChats aqui novamente, pois já foi feito no loginUserUIUpdate
            // e o atributo data-chat-id ajuda a manter o estado ativo.
        }
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Configurar linguagem (ainda simulado no frontend)
    document.querySelectorAll('.language-option').forEach(option => {
        option.addEventListener('click', () => {
            ...
            // Aqui você implementaria a mudança de idioma real se necessário
